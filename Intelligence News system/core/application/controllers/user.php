...
class User extends Base_e_army {
        public function __construct() {
            ...
            $this->images_path = $this->config->item('root_upload').$this->config->item('images_path_user');
	...
	public function lists($offset=0,$ua_username = NULL) {
		...
		$check = 0;
                ...
                if($check == 1){
                    ...
                    if(isset($_POST['rtd_unit']) && $_POST['rtd_unit'] == 'many'){
                        if(isset($_POST['ckunit']) && $_POST['ckunit']){
                            $_POST['u_unitid'] = "";
                            foreach ($_POST['ckunit'] as $k_c => $v_c){
                                $_POST['u_unitid'] .= $v_c."9";
                            }
                            //$_POST['u_unitid'] = $_POST['ckunit'];
                        }else{
                            redirect('user/lists/0/'.$_POST['ua_username']);
                        }
                        if(isset($_POST['rtd_ckunit']) && $_POST['rtd_ckunit']){
                            $_POST['ua_mainunit'] = $_POST['rtd_ckunit'];
                        }
                    }
                    unset($_POST['rtd_unit']);
                    unset($_POST['ckunit']);
                    unset($_POST['rtd_ckunit']);
                    
                    $_POST['ua_username'] = trim($_POST['ua_username']);
                    //$_POST['ua_dbusername'] = strtoupper($_POST['ua_username']);
                    $_POST['ua_logincount'] = "0";
                    //$_POST['ua_approvercode'] = "1234";
                    $_POST['ua_password'] = encrypt(trim($_POST['ua_password']), $_POST['ua_username']); // when need encrypt
                    ...
                    /*---------------------------------------- start insert img ------------------------------------------*/
                $user_id_path = $this->images_path.$id_user;
                //echo $user_id_path;
                if (!file_exists($user_id_path.'/')) {
                    mkdir($user_id_path, 0775);
                }
                
                if (isset($_FILES['image_file']) and $_FILES['image_file']['error'] == 0) {
                    $this->custom_upload->setConfig(array('upload_path' => $user_id_path.'/'));
                    ...
                    $imgupload['ua_mainimage'] = (isset($upload_data['orig_name']) and $upload_data['orig_name']) ? $upload_data['orig_name'] : NULL;
                    $this->m_user->edit('user_account', array('ua_userid' => $id_user), array('ua_mainimage' => $imgupload['ua_mainimage']));
                }
                /*----------------------------------------- end insert img -------------------------------------------*/ 
        ...
        public function edit($id = null){
            ...
            if(!isset($_POST['ua_userid']) && empty($_POST['ua_userid'])){
                ...
                $data_result['detail'] = $this->m_user->getDetail('user_account',$data);
                
                $data_result['detail']['ua_password'] = decrypt($data_result['detail']['ua_password'], $data_result['detail']['ua_username']);
                
                $this->view('user/v_user_edit_profile', $data_result);
            }else{
                 $data['ua_userid'] = $id;
                 $_POST['ua_password'] = encrypt(trim($_POST['ua_password']), $_POST['ua_username']); // when need encrypt
                 if(isset($_POST['rtd_unit']) && $_POST['rtd_unit'] == 'many'){
                 ...
                 /*---------------------------------------- start insert img ------------------------------------------*/
                $user_id_path = $this->images_path.$id;
                if (!file_exists($user_id_path.'/')) {
                    mkdir($user_id_path, 0775);
                }
                if (isset($_FILES['image_file']) and $_FILES['image_file']['error'] == 0) {
                    ...
                    if (isset($upload_data) and $upload_data and $this->data['detail']['ua_mainimage']) {
                        delete_file($user_id_path . '/' . $this->data['detail']['ua_mainimage']);
                    }
                    $imgupload['ua_mainimage'] = (isset($upload_data['orig_name']) and $upload_data['orig_name']) ? $upload_data['orig_name'] : NULL;
                    $this->m_user->edit('user_account', array('ua_userid' => $id), array('ua_mainimage' => $imgupload['ua_mainimage']));
                }
                /*----------------------------------------- end insert img -------------------------------------------*/ 
                 redirect('user/lists');
            }
        ...
        public function Edit_profile($id = null){
            ...
            $this->data['breadcrumb'] = array(array('name' => 'หน้าแรก', 'link' => site_url('')), array('name' => 'แก้ไขข้อมูลส่วนบุคคล'));
                ...
                $data_result['unit'] = $this->data['unit'] = $this->m_user->getAll('unit', NULL, NULL, 'u_unitid asc');
                $data_result['unit_sub'] = $this->data['unit_sub'] = $this->m_user->getAll('unit_sub', NULL, NULL, 's_unitsub_id asc');
                $data_result['user_group'] = $this->data['user_group'] = $this->m_user->getAll('user_group');
