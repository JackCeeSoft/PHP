...
class Jq_upload_person extends Base_e_army  {
    ...
    public function __construct() {
        ...
        $this->images_path = $this->config->item('root_upload').$this->config->item('images_path_person_gallery');
        ...
        //Delete img url
        $this->setDelete_img_url(base_url().'jq_upload_person/deleteImage/');
    ...
    public function upload_img($id = '') {
        ...
        $result = $this->m_db->getDetail('person', array('p_personid' => $id));
        ...
        if ($this->do_upload()) {
            ...
            $person_name = $uni_name . $data['file_ext'];
            $person_thumb_name = $uni_name . '_thumb';
            ...
            //Insert data
            $insert['p_personid'] = $id;
            $insert['pi_path'] = $person_name;
            $insert['pi_createdby'] = $sesstion['user_id'];
            $insert['pi_updatedby'] = $sesstion['user_id'];
            $insert_id = $this->m_db->add('person_imageattach', $insert);
            ...
            $info->url = $this->getPath_img_upload_folder() . $person_name;
    ...
    public function deleteImage($id) {
        ...
        $data = $this->m_db->getDetail('person_imageattach', array('pi_imageattachid' => $id));

        list($name, $ext) = explode('.', $data['pi_path']);
        $thumb = $name.'_thumb.'.$ext;
        $success = unlink($this->getPath_img_upload_folder() . $data['p_personid'] . '/' . $data['pi_path']);
        //$success_th = unlink($this->getPath_img_upload_folder() . $data['n_newsid'] . '/' . $thumb);

        $this->m_db->delete('person_imageattach', array('pi_imageattachid' => $id));

        //info to see if it is doing what it is supposed to
        $info = new stdClass();
        $info->sucess = $success;
        $info->path = $this->getPath_url_img_upload_folder() . $data['p_personid'] . '/' . $data['pi_path'];
        $info->file = is_file($this->getPath_img_upload_folder() . $data['p_personid'] . '/' . $data['pi_path']);
