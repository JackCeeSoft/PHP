...
/*    public function search_news($word = NULL, $where = NULL ,$limit = NULL, $offset = NULL,$not_word = null,$where_or_and = null , $notwhere_or_and=null){
            $array_data = array();
            $count_news = 0;
            $n_id = 0;
            $and_or = " ";
            ...
            if($where != NULL && isset($where['u_unitid']) && $where['u_unitid'] != ""){
                ...
                if($where_or_and == 'OR')$cmd_count_news .= "(";
            }
            ...
            if($word != null){
                foreach ($word as $key_word){
                    if($key_word == ""){
                        ...
                        $cmd_count_news .= "n.n_writer like '%".$key_word."%' "
                                . " OR n.n_subject like '%".$key_word."%' "
                                . " OR np.np_paragraph like '%".$key_word."%' ".$and_or.$where_or_and." ";
                    }  
                }
                if($where_or_and == 'OR'){
            ...
            if($where != NULL && isset($where['u_unitid']) && $where['u_unitid'] != ""){
                ...
                if($where_or_and == 'OR')$cmd_limit_news .= "(";
            }
            ...
            if($word != null){
                foreach ($word as $key_word){
                    if($key_word == ""){
                        ...
                        $cmd_limit_news .= "n.n_writer like '%".$key_word."%' "
                                . " OR n.n_subject like '%".$key_word."%' "
                                . " OR np.np_paragraph like '%".$key_word."%' ".$and_or.$where_or_and." ";
                    }  
                }
...
*/
    public function search_news($word = NULL, $where = NULL ,$limit = NULL, $offset = NULL,$not_word = null,$where_or_and = null , $notwhere_or_and=null){
            ...
            if($not_word != null && $not_word){
                if($notwhere_or_and == 'AND')$cmd_count_news = substr($cmd_count_news,0,-1);
                //if($where_or_and == 'AND')$cmd_count_news .= '(';
                foreach ($not_word as $not_key_word){
                    if($not_key_word == ""){

                    }else{
                        if($notwhere_or_and == 'AND')
                        {
                            $cmd_count_news .= "(";
                            $and_or_dont = ")";
                        }
                        $cmd_count_news .= "n.n_writer not like '%".$not_key_word."%' "
                                . " AND n.n_subject not like '%".$not_key_word."%' "
                                . " AND np.np_paragraph not like '%".$not_key_word."%' ".$and_or_dont.$notwhere_or_and." ";
                    } 
                }
                
                if($notwhere_or_and == 'OR'){
                    $cmd_count_news = substr($cmd_count_news, 0,-3);
                }else{
                    $cmd_count_news = substr($cmd_count_news, 0,-4);
                }
                
                if($notwhere_or_and == 'OR')$cmd_count_news .= ")";
                //END
                $cmd_count_news .= ' AND ' ;
                if($where_or_and == 'OR')$cmd_count_news .= "(";
            ...
            if($not_word != null && $not_word){
                if($where_or_and == 'OR')$cmd_limit_news = substr($cmd_limit_news,0,-1);
                if($notwhere_or_and == 'OR')$cmd_limit_news .= '(';
                ...
                foreach ($not_word as $not_key_word){
                    if($not_key_word == ""){
                        ...
                        $cmd_limit_news .= "n.n_writer not like '%".$not_key_word."%' "
                                . " AND n.n_subject not like '%".$not_key_word."%' "
                                . " AND np.np_paragraph not like '%".$not_key_word."%' ".$and_or_dont.$notwhere_or_and." ";
                    } 
                }
            ...
            if($word != null){
                ...
                $cmd_limit_news .= " group by n.n_newsid order by n.n_date desc, n.n_time desc LIMIT ".$limit." OFFSET ".$offset."";
            }else{
                if($not_word != null){
                    $cmd_limit_news = substr($cmd_limit_news, 0,-5);
                    $cmd_limit_news .= " group by n.n_newsid";
                }else{
                    $cmd_limit_news = substr($cmd_limit_news, 0,-5);
                    $cmd_limit_news .= " group by n.n_newsid order by n.n_date desc, n.n_time desc LIMIT ".$limit." OFFSET ".$offset."";
                }
            ...
            if($not_word != null && $not_word){
                if($where_or_and == 'OR')$cmd = substr($cmd,0,-1);
                if($notwhere_or_and == 'OR')$cmd .= '(';
                foreach ($not_word as $not_key_word){
                    if($not_key_word == ""){
                        ...
                        $cmd .= "n.n_writer not like '%".$not_key_word."%' "
                                . " AND n.n_subject not like '%".$not_key_word."%' "
                                . " AND np.np_paragraph not like '%".$not_key_word."%' ".$and_or_dont.$notwhere_or_and." ";
                    } 
                }
            ...
            $cmd .= " order by n.n_date desc, n.n_time desc";
						...
						$cmd_count_news .= "np.".$key." = '".$values."' and ";
                                                ...
                                                if(is_array($values)){
                                                    foreach ($values as $values_data){
                                                        $cmd_count_news .= "n.".$key." like '%".$values_data."%' and ";
                                                        //print_r($values_data);
                                                    }
                                                }else{
            ...
            foreach ($word as $key => $values){     
                if(strpos($key,'n_datetime') !== false){
					...
					if ($key == "nd_newsdepartmentid") {
						...
						if(is_array($values)){
                                                    foreach ($values as $values_data){
                                                        $cmd_limit_news .= "n.".$key." like '%".$values_data."%' and ";
                                                        //print_r($values_data);
                                                    }
                                                }else{
            ...
            foreach ($word as $key => $values){
                if(strpos($key,'n_datetime') !== false){
					...
					if ($key == "nd_newsdepartmentid") {
						...
						if(is_array($values)){
                                                    foreach ($values as $values_data){
                                                        $cmd .= "n.".$key." like '%".$values_data."%' and ";
                                                        //print_r($values_data);
                                                    }
                                                }else{
                                                    $cmd .= "n.".$key." = '".$values."' and ";
                                                }
					}					
                }
            }
